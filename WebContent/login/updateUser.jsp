	...
	<div class="innerWrap signWrap">
		<div class="form">
			...
			<form name="writeForm" method="post" action="${rootPath}/join?command=update" onSubmit='return checkValid()'>
				<div>
					<label>아이디</label>
					<div><input type=text name="userId" value="${sessionUser.userId}" readonly></div>
				</div>
				<div>
					<label>비밀번호</label>
				...
				<div>
					<label>이름</label>
					<div><input type=text name="name" value="${sessionUser.name}" readonly></div>
				</div>
				<div>
					<label>이메일</label>
					<div><input type="email" name="email" value="${sessionUser.email }"></div>
				</div>
				<div class="btn">
